...
desc "setup and load environment"
task :environment do
  invoke :'rbenv:load'
end

task :pwd do
  queue %[cd #{settings.deploy_to}]
end

task :working_directory do
  invoke :pwd
  queue "cd #{application_name}"
end

namespace :app do

  desc "setup server for first time"
  task :setup => :environment do
    queue %[sudo ruby -e 'Dir.mkdir("/var/server") if !File.directory?("/var/server")']
    queue %[sudo chown deployer:deployer /var/server]
    invoke :pwd
